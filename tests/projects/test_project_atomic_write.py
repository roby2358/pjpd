...
@pytest.fixture()
def done_file(tmp_path: Path) -> Path:
    return tmp_path / "project_done.txt"
...
def test_project_save_tasks_uses_text_records(project_file: Path, done_file: Path) -> None:
    """Project._save_tasks should use TextRecords.write_atomic."""
    project = Project(name="TestProject", file_path=project_file, done_file_path=done_file)
...
def test_project_save_tasks_creates_file_when_nonexistent(project_file_nonexistent: Path, done_file: Path) -> None:
    """Project._save_tasks should create the file when it doesn't exist initially."""
    project = Project(name="TestProject", file_path=project_file_nonexistent, done_file_path=done_file)
